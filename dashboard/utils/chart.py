...
class LineCross():
        ...
        if cond_array:
            if len(self.dc_date_ls):  # 마지막 cross 날짜 가져옴
                if len(self.line1.df_last_low_points):
                    df_the_date = self.line1.df_last_low_points.loc[self.gc_date_ls[-1]:]  # gc이후의 저점 데이터 가져옴. 
                    ## 1개이면 첫번째 v 
                    print(f"조건만족 저점개수: {len(df_the_date)} 개 in is_gcv") if verbose else None
                    print(f"조건 저점일 데이터{df_the_date} in is_gcv") if verbose else None
    ...
    def __get_width(self):
        ...
        width_s = lines.apply(
            lambda x: (max(x[self.__s1_name], x[self.__s2_name]) / min(x[self.__s1_name], x[self.__s2_name]) - 1) * 100, axis=1
        )
        width_s.name = 'width'
        return width_s
...
class Line():
    ...
    def __init__(self, data, start=None, end=None, ignore_cnt=1, allow_rate=1):
        ...
        self._value_column ,self._change_column=  f'{self.name}_변곡점',f'{self.name}_변곡점_변화량'
        
        self.current_value = self.data.iloc[-1]
        self.current_direction = "up" if data.iloc[-2] < self.data.iloc[-1] and data.iloc[-3] < self.data.iloc[-1] else "down" # 방향. 2일 연속 만족해야 인정.
            
        self.df_curve = pd.DataFrame(self.data)
        self.df_curve[f"{self.name}_변화량"] = self.data.pct_change()
        self.df_curve = pd.concat([self.df_curve, self._add_low_high(self.df_curve[self.name], ignore_cnt=self._ignore_cnt)], axis=1)
        self.df_all_low_points, self.df_all_high_points, self.df_last_low_points = self._get_low_high_points(allow_rate=self._allow_rate)
        ...
        self.inclination20_value = round(self.inclination20.iloc[-1] * 100, 1)## 편평도. 
        self.inclination10_value = round(self.inclination10.iloc[-1] * 100, 1)## 편평도. 
    ...
    def _add_low_high(self, series, ignore_cnt=0):
        ...
        variation_cond = ((series1.shift(-1) < series1) & (series1 > series1.shift(1))) | ((series1.shift(-1) > series1) & (series1 < series1.shift(1)))
        ...
        # result_df = pd.concat([series, result], axis=1)
            ...
            result[[f"{series.name}_변곡점" ,f"{series.name}_변곡점_변화량"]] = np.nan, np.nan
    ...
    def _get_low_high_points(self, allow_rate=1):
        ...
        if not len(df):
            all_low_points, all_high_points, last_low_points  = pd.Series(), pd.Series(), pd.Series()
        ...
        all_low_points = df1.filter([self._value_column, self._change_column]).loc[cond_low] # DataFrame
        all_high_points = df1.filter([self._value_column, self._change_column]).loc[cond_high] # DataFrame
        ...
        l = low_df[f"{self._value_column}"].shift(1) * allow_rate < low_df[f"{self._value_column}"] 
        l.name = "left"
        r = low_df[f"{self._value_column}"] * allow_rate  < low_df[f"{self._value_column}"].shift(-1) 
        r.name= 'right'
    ...
    def is_w(self, verbose=False):
        ...
        if len(self.df_last_low_points):
            w_cond1 = self.df_last_low_points.index[-1] == self.df_all_low_points.index[-1] # 마지막w가 현재 진행중이다.
            w_cond2 = len(self.df_last_low_points) > 1  # 마지막 저점리스트가 1개 이상이다.
            w_cond3 = self.df_all_high_points.index[-1] < self.df_all_low_points.index[-1] if len(self.df_all_high_points) else False  # 최근고점보다 최근저점이 크다.
            ...
            if verbose:
                ...
                print(f'지속일 :{len(self.data.loc[self.df_last_low_points.index[-1]:])} 일 ')
                print(f"현재이격도: {round(self.disparity[-1],2)}" )
        if w_cond:
            return len(self.data.loc[self.df_last_low_points.index[-1]:]) ## 지속일 반환.
        else:
            return 0
    ...
    def is_ab_period(self, verbose=False):
        ...
        result = False
        if self.is_w():
            a = self.df_curve.loc[self.df_all_low_points.index[-2] :self.df_all_high_points.index[-1]]
            b = self.df_curve.loc[self.df_all_high_points.index[-1] :self.df_all_low_points.index[-1]]
            result = len(a) >= len(b)
            if result:
                print(f"상태: w", ) if verbose else None
            ...
            a = self.df_curve.loc[self.df_all_low_points.index[-1] :self.df_all_high_points.index[-1]]
...
class Ma(Line):
    ...
    def __init__(self, df, ma=3 , start=None, end=None, allow_rate=1, ignore_cnt=0):
        ...
        super().__init__(data = df[self.str_ma],  ignore_cnt=ignore_cnt, allow_rate=allow_rate)
        
        self.df_curve = pd.concat([df.filter(regex=f"^(?!.*{self.str_ma}).*"), self.df_curve],axis=1)
    ...
    def __adjust_columns(self, df):
        new_name_dict = {'open':'Open', 'high':'High','low':'Low','close':'Close',
                         'volume':'Volume','value':'Value','change':'Change',}
    ...
    def _get_전저점(self):
        ...
        if len(self.df_all_low_points):
            ...
            try:
                except_word  ='ma\d+'
                if not re.findall(f"^(?!.*{except_word}).*", self.name): ## ma가 아닌게 아니면. -> ma이면   ex) ma40 (o)  rsi(x)
...
class Ac111():
    ...
    def _get_ac_info(self, df):
        ...
        if len(df):
            # 
            ac_cond1 = (df['Close'] > df['Close'].shift(1)) & ((df['Volume'] > df['Volume'].shift(1) * self._pre_vol_rate ) | (df['Volume'] > df['Volume'].shift(2) * self._pre_vol_rate ))
            ac_cond2 = (df['Close'] > df['Close'].shift(1)) & ((df['Volume'] > df['vol20ma'] * self._ma20vol_rate) | (df['Volume'] > df['vol240ma'] * self._ma20vol_rate * 1.5 ))
            ac_cond3 = (df['Close'] > df['Open']) & ((df['Volume'] > df['Volume'].shift(1) * 1.5 ) | (df['Volume'] > df['Volume'].shift(2) * 1.5 ))
            ...
            if len(through_mas):
                for ma in through_mas:
                    ...
                    if re.match(pattern, ma):
                        ma = f"ma{ma}"
                    if ma in df.columns:
                        through_cond = (df['Low'] < df[f'{ma}']) & (df[f'{ma}'] < df['Close'] )
                        ...
                        if re.match('^upper.+$', ma):
                            ...
                            flat_rate = pd.concat([df[ma].pct_change(20),df[ma].pct_change(10)], axis=1).mean(axis=1)
...
class BB():
    ...
    def __get_line(self, df, ma):
        ...
        if used_ma:
            ...
            self.upper.name , self.middle.name , self.lower.name = f"upper",f"middle",f"lower"
    ...
    def __get_info(self):
        ...
        if self.exist :
            ...
            self.upper_direction = "up" if self.upper.iloc[-2] < self.upper.iloc[-1] else "down" # 방향.
            self.upper_inclination20 = self.line_upper.inclination20_value
            self.upper_inclination10 = self.line_upper.inclination10_value
            
            self.lower_direction = "up" if self.upper.iloc[-2] < self.upper.iloc[-1] else "down" # 방향.
...
class Sun():
    ...
    def __init__(self, df,start=None, end=None):
        ...
        for i in range(10, min(len(df), 200) + 1, 10):  ## 최고 200까지만 보기로 함. ??????
...
class Rsi():
    ...
    def __init__(self, df, period=11, low_value = 40 , high_value = 70, start=None, end=None):
    ...
    def is_rsi_w(self,verbose=False):
        ...
        # w 이고 
        cond_v = (len(self.line_rsi.df_last_low_points) > 0) and (self.line_rsi.df_last_low_points.index[-1] == self.line_rsi.df_all_low_points.index[-1])
        
        # w 최근두번중 한번 low_value 에 빠졌다 나온것 
        cond_value = sum(self.line_rsi.df_last_low_points[f"{self.line_rsi.name}_변곡점"][-2:] < self._low_value) > 0
        ...
        if verbose:
            print(f"지속일 : {len(self.data.loc[self.line_rsi.df_last_low_points.index[-1]:])} candle")
            print(f"현재값 : {self.current_value:,.1f}")
            print(f"기준값 : {self._low_value:,.1f}")
            print(f"최근 최저점값 : {self.line_rsi.df_last_low_points['rsi_변곡점'].iloc[-2:].min():,.1f}" )
    ...
    def plot(self):
        ...
        ax.hlines([self._low_value, self._high_value], self.line_rsi.df_curve.index[0],self.line_rsi.df_curve.index[-1],colors='red')
        return ax
...
class Ac():
    ...
    def __init__(self, df, pre_vol_rate=2, ma20vol_rate =2, start=None, end=None):
        ...
        self.ac_dates = self._get_ac_date(pre_vol_rate=self._pre_vol_rate,ma20vol_rate=self._ma20vol_rate ) ## default
    ...
    def _get_ac_date(self, pre_vol_rate, ma20vol_rate):
        ...
        if len(self.df):
            ...
            cond_전일대비거래량비율 = (self.df['Volume'] > self.df['Volume'].shift(1) * pre_vol_rate )
            cond_전전일대비거래량비율 = (self.df['Volume'] > self.df['Volume'].shift(2) * pre_vol_rate )
            cond_전일대비거래량비율_handycap = (self.df['Volume'] > self.df['Volume'].shift(1) * pre_vol_rate * handycap )
            cond_전전일대비거래량비율_handycap = (self.df['Volume'] > self.df['Volume'].shift(2) * pre_vol_rate * handycap)
            
            cond_20평균대비거래량비율 = (self.df['Volume'] > self.vol20.ma_vol * ma20vol_rate) if 'vol20' in dir(self) else False
            cond_240평균대비거래량비율 = (self.df['Volume'] > self.vol240.ma_vol * ma20vol_rate * 1.5 ) if 'vol240' in dir(self) else False
            
            ac_cond1 = cond_pre_plus & (cond_전일대비거래량비율 | cond_전전일대비거래량비율 )
            
            ac_cond2_1 = (cond_pre_plus & cond_20평균대비거래량비율) 
            ac_cond2 = cond_pre_plus & (cond_20평균대비거래량비율 | cond_240평균대비거래량비율) if 'vol240' in dir(self) else ac_cond2_1
            
            # 전전일과 전일 조건 모두 만족할때. default의 3/4 적용.
            ac_cond3 = cond_plus & (cond_전일대비거래량비율_handycap & cond_전전일대비거래량비율_handycap)
...
class Volume():
    ...
    def is_ab_by_ma(self, ma=20, period=50, verbose=False):
        ...
        if len(v_df1):
            ...
            print(f"{the_date} ma20:{result_rate}배 , a:{a_vol_avg:,.0f}, b:{b_vol_avg:,.0f}, last5:{last5_vol_avg:,.0f}") if verbose else None
        return result_bool
    ...
    def is_ab_by_big_v(self, period=50,verbose =False):
        ...
        result_bool = False
        period_date= self.df.index[-period]
        big_v = (self.df['Volume'] > self.ma_vol * self.big_v_rate) & (self.df['Open'].shift(1) < self.df['Open'])
        ...
        if len(big_v):
            ...
            print(f"{the_date} big:{result_rate}배 , a:{a_vol_avg:,.0f}, b:{b_vol_avg:,.0f}, last5:{last5_vol_avg:,.0f}") if verbose else None
            # print(the_date, 'big', result_rate) if verbose else None
        return result_bool
        
    
    def is_ab(self, period=50, verbose=False):
        return any([self.is_ab_by_big_v(period=period, verbose=verbose), self.is_ab_by_ma(period=period, verbose=verbose)])
...
class Candle():
    ...
    def get_candle_info(self, n봉전=0):
        ...
        c_data = self.__candle_status(data['Open'],data['High'],data['Low'],data['Close'])
        return c_data
        
        
    def get_short_candle_date(self, n:int = 30, quantile_rate:float=0.25, verbose=False) -> list[datetime]:
        '''
        n일 이내
        quantile_rate 0 ~ 1 
        '''
        data  = self.df.apply(lambda x : self.__candle_status(x['Open'],x['High'],x['Low'],x['Close'])['body'], axis=1)[-n:]
        short_dates = data.loc[data < data.quantile(quantile_rate)].index
        print(f"최근 {n}일간 short_candle {len(short_dates)}개 (quantile_rate:{quantile_rate})") if verbose else None
        return short_dates
    ...
    def is_바닥캔들_status(self, option = 'W',verbose=False) -> dict:
        ...
        '''
        if option =='W':
            df = self.df.resample('W').agg({'Open': 'first', 'High': 'max', 'Low': 'min', 'Close': 'last', 'Volume': 'sum'})
        elif option =='M':
            df = self.df.resample('ME').agg({'Open': 'first', 'High': 'max', 'Low': 'min', 'Close': 'last', 'Volume': 'sum'})
        elif option =='':
            df = self.df
        ...
        try:
            first_candle = temp_df.iloc[-5].to_dict()
            second_candle = {'Open': temp_df.iloc[-4:-2]['Open'].iloc[0],
        ...
        if len(dic):
            for key, value in dic.items():
                ...
                세번째봉_전고돌파조건 = max(value['2']['Close'],value['2']['Open']) <  value['3']['Close'] # 2캔들보다 3종가가 큼.
    ...
    def plot(self, option='W',recent_high=None, title=""):
        ...
        * recent_high: 이전 고점 표시 여부 혹은 N일전까지 고점 표시 (-1 이면 어제까지 고점표시)
        ...
        if option =='W':
            ...
            df = self.df.resample('M').agg({'Open': 'first', 'High': 'max', 'Low': 'min', 'Close': 'last', 'Volume': 'sum'})
        elif option =='':
            df = self.df
        ...
        x = np.arange(len(df))
        pp = figure(title=str(title), plot_width=750, plot_height=300, min_width=750, min_height=300, x_range=(max(1, len(df)-15), max(15, len(df))))
        pp.segment(x[inc], df.High[inc], x[inc], df.Low[inc], color="red")
        pp.segment(x[dec], df.High[dec], x[dec], df.Low[dec], color="blue")
        pp.vbar(x[inc], 0.8, df.Open[inc], df.Close[inc], fill_color="red", line_color="red")
        pp.vbar(x[dec], 0.8, df.Open[dec], df.Close[dec], fill_color="blue", line_color="blue")
        pp.yaxis[0].formatter = NumeralTickFormatter(format='0,0')
        pp.xaxis.visible = False
        
        pp.line(x, df['ma3'].reset_index(drop=True), line_width=1, line_color="black") if len(df['ma3']) else None
        pp.line(x, df['ma5'].reset_index(drop=True), line_width=2, line_color="red") if len(df['ma5']) else None
...
class PriceLevel():
    ...
    def _get_price_level(self, df, parts=10, period=240):
        ...
        result = df.groupby("bin", observed=True).sum()["rate_v"].sort_values(ascending=False)

        return [int(index) for index in result.index[0:2]]

class Chart():
    
    def __init__(self, df:pd.DataFrame=None, mas=None, start=None, end=None, n봉=0, *args,  **kwargs):
        '''
        df : kind ohlcv,
        ...
        ignore_cnt_dict = {1:0, 3:1, 5:1, 10:2, 20:2, 40:2, 60:3, 120:4, 240:5,}
        ## ma setting
        if len(df):
            ...
            if self.ma_arr:
                for ma in self.ma_arr:
                    if ma in ignore_cnt_dict:
                        ...
                        ignore_cnt = 1
                    setattr(self , f"ma{ma}", Ma(df, ma, start=self.__start, end= self.__end, ignore_cnt=ignore_cnt))
        ## ma1 추가
        ...
        try:
            self.rsi = Rsi(df,period=11, low_value=40,high_value=70, start=self.__start, end= self.__end)
        except Exception as e:
            print('rsi 생성실패', e)
    ...
    def is_w_a_short(self, ma=3, with_vol=True, verbose=False):
        ...
        if with_vol and len(ma_instance.df_all_low_points.index) >=2:
            ...
            vol_cond = sum(check_data) > 0 and sum(check_data[-2:]) <= 1  ## 전저점기준으로 평균거래량 이상인게 있고 -1일 또는 -2 일 저점이있는경우. 
        else:
            the_date = []
            vol_cond = True
        
        print(f"cond_w {cond_w} cond_wa {cond_wa} cond_short {cond_short} with_vol {vol_cond} the_date: {the_date}") if verbose else None
    ...
    def is_ab(self, ma:int=3, verbose=False):
        ...
        result = False
        ma_instance : Ma = getattr(self, f"ma{ma}" if isinstance(ma, int) else ma)
        result = ma_instance.is_ab_value(verbose=verbose) & ma_instance.is_ab_period(verbose=verbose)
    ...
    def w3_ac(self, verbose=False):
        ...
        try:
            ...
            cond1 = ma3.is_w(verbose=verbose) or ma3.is_wa(verbose=verbose) or ma5.is_w(verbose=verbose) or ma5.is_wa(verbose=verbose)
            cond2 = ac.is_ac_today(verbose=verbose)
            cond3 = max(ma3.data.iloc[-1], ma5.data.iloc[-1]) < self.df['Close'].iloc[-1]
            result = cond1 & cond2 & cond3
        except Exception as e:
            print("chart.w3_ac() 에서 오류",  e)
        
        
        print(f"is_w(a): {cond1},  is_ac : {cond2} locate_cur_close_with_short_ma : {cond3}") if verbose else None
        return result
    ...
    def is_good_array(self, option='normal', verbose=False):
        ...
        result_ls = [datas[a] > datas[b] for a, b in itertools.combinations(datas.keys(),2) ]
        if option=='perpect':
            # 방향까지 전부 우상향인것.
            direction_datas = {f"ma{ma}": getattr(self, f"ma{ma}").current_direction for ma in check_mas }
            result_directions = [True if v =='up' else False for k, v in direction_datas.items() ]
    ...
    #sun_ac
    def sun_ac(self, sun_width=30, with_ac=True, n봉전이내=0, verbose=False):
        '''
        ac 이고 꼬리 까지 확인하기. 
        '''
        ...
        for n봉전 in range(n봉전이내+1):
            n = -(n봉전)-1
            cond_through = self.df['Low'].iloc[n] < self.sun.two_line.line1.data.iloc[n-1] < self.df['Close'].iloc[n]
            ...
            if with_ac:
                ...
                candle_rate = sum([v for k, v in self.candle.get_candle_info(n봉전=n봉전).items() if k in ['body','dow_tail']])
    ...
    # coke_ac
    def coke_ac(self , coke_width=60, period = 240, with_ac=True, n봉전이내=0, verbose=False):
        '''
            ...
            if period ==240:
                if 'bb240' in dir(self):
                    ...
                    if 'two_line' in dir(self.bb240):
                        cond_through = self.df['Low'].iloc[n] < self.bb240.upper.iloc[n-1] < self.df['Close'].iloc[n]
                ...
                if 'bb60' in dir(self):
                    ...
                    if 'two_line' in dir(self.bb60):
                        cond_through = self.df['Low'].iloc[n] < self.bb60.upper.iloc[n-1] < self.df['Close'].iloc[n]
    ...
    def multi_through(self, with_ac=True, n봉전이내=0,verbose=False):
        '''
        ...
        if all(exists_conds):
            ...
            for n봉전 in range(n봉전이내+1):
                ...
                if pd.notna(ma20.data[n]) and pd.notna(ma60.data[n]):
                    ...
                    cond_through1 = self.df['Low'].iloc[n] < ma20_current_value <  self.df['Close'].iloc[n] 
                    cond_through2 = self.df['Low'].iloc[n] < ma60_current_value <  self.df['Close'].iloc[n] 
                    cond_ac = True
    ...
    def is_alphabeta_status(self, cross_mas = [60,120,240], verbose=False):
        ...
        cross_arr_str = [(f"ma{item[0]}",f"ma{item[1]}", ) for item in itertools.combinations(cross_mas, 2) if item[0] < item[-1]]
        long_cross_dates = []
        ...
        if long_cross_dates:
            ...
            if cond_list:
                ...
                max_long_value = max(list(set([getattr(self,item).data.iloc[-1] for items in cond_list for item in items[0]])))
    ...
    def is_abc(self, verbose=False):
        ...
        # 큰거래중 4프로이상만 추출
        big_ac_df = self.volume_big.big_volume_df.loc[self.volume_big.big_volume_df['Change'] >= 0.04]
        ...
        if self.is_w_a_short(with_vol=False):
            ...
            if len(exist_df):
                ...
                after_data_big = self.ma3.data.loc[last_big_vol_date:] #빅거래이후 ma3 추이. 
                if not len(after_data_big.loc[after_data_big < 최소기준]):
                
                    result = True
                    print('최소기준아래로 떨어진데이터', after_data_big.loc[after_data_big < 최소기준]) if verbose else None
    ...
    def is_coke_gcv(self,ma=3,  bb_ma=240, with_ac=True, bb_width= 50, verbose=False):
        ...
        try:
            ...
            if with_ac:
                try:
                    end_date = two.last_cross_date
                    start_date =  short_ma_line.df_last_low_points.loc[:end_date].index[-1]  
                    # big_vol_df = self.volume.big_volume_df.loc[start_date: end_date]
                    ac_dates= self.ac.ac_dates[(self.ac.ac_dates >= start_date) & (self.ac.ac_dates <= end_date)]
    ...
    def is_rsi(self, option='new_phase', verbose=False):
        ...
        if option=='new_phase':
            ...
            option_cond = self.is_new_phase(verbose=verbose) & self.is_good_array('perfect')
        elif option=='any':
            option_cond = self.is_new_phase(verbose=verbose) | self.is_good_array('perfect')
        else:
            print("지정된 option 이 올바르지 않는다. 'new_phase', 'good_array', 'all', 'any' ")
            return False
    ...
    def is_new_phase(self, verbose=False) -> bool:
        ...
        cond_coke_up2 = (temp_df["Close"].shift(1) < upper_data) & (temp_df["Close"] > upper_data)
        ...
        all_cond = (cond_coke_up1 | cond_coke_up2) & (cond_coke_width | change_rate_cond)
        ...
        #     cond_price = False
            ...
            try: 
                cond_current_status1 = self.ma3.data.iloc[-1] >= self.ma60.data.iloc[-1]  ## 아직 3선이 60선위에 있는것만.
            except:
                try:
                    cond_current_status1 = self.ma3.data.iloc[-1] >= self.ma20.data.iloc[-1]
                except:
                    print('ma 오류')
                    cond_current_status1 = False
                    
            try:
                cond_current_status2 = (self.ma3.is_w() | self.ma5.is_w()) & (self.ma3.data.iloc[-1] >= self.ma240.data.iloc[-1])
            except:
                cond_current_status2 = False
            
            cond_current_status = (cond_current_status1 | cond_current_status2) # 60선위에 있거나 240위에서 w를 그린것.
    ...
    def plot_bokeh(self, **kwargs):
        ...
        df['candle_color'] = ["#f4292f" if row['open'] <= row['close'] else "#2a79e2" 
        ...
        df['vol_color'] = ['blue' if bl else value 
                        for value, bl in zip(df['vol_color'], low_cond)]
        
        if 유동주식수:
            cond_유통주식수 = df['volume'] >= 유동주식수 
            if sum(cond_유통주식수):
                print('1')
                df['vol_color'] = ["#ffff00" if bl else value 
        ...
        if 상장주식수:
            ...
            if sum(cond_상장주식수):
                print('2')
                df['vol_color'] = ['#800080' if bl else value 
        ...
        ###### plot ####################################################
        plot_option = dict(width=800,
                        height=300,
                        background_fill_color="#ffffff",
                        )
        all_tools = [    
        "PanTool","WheelZoomTool","BoxZoomTool","ResetTool",
        "SaveTool","CrosshairTool","HoverTool","TapTool",
        "LassoSelectTool","PolySelectTool","ZoomInTool",
        "ZoomOutTool","FreehandDrawTool","EditTool","BoxSelectTool",    
        ]
        ...
        p1.x_range.end = source.data['index'][-1] + 30 #+ pd.Timedelta(days=1)  # 마지막 데이터 포인트 이후로 범위 확장
        ...
        p2 = figure(**plot_option, x_range=p1.x_range,)  # x_range를 공유해서 차트가 일치하게 만듦
        p2.vbar(x="index",
                top="volume",
                width=0.7,
        ...
        # ## 주석 (유통주식수, 상장주식수) Line 
        if 유동주식수:
            p2_유통주식수 = Span(location=유동주식수, dimension='width', line_color='#9932cc', line_width=1)
            p2.add_layout(p2_유통주식수)
        if 상장주식수:
            p2_상장주식수 = Span(location=상장주식수, dimension='width', line_color='#8b008b', line_width=1)
        ...
        p1.yaxis.formatter = NumeralTickFormatter(format="0,0")  # 천 단위 쉼표 형식 적용
        p2.yaxis.formatter = NumeralTickFormatter(format="0,0")  # 천 단위 쉼표 형식 적용
